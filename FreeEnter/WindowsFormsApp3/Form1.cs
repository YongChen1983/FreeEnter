...
{
    public partial class FreeEnter : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            TopMost = true;
            LoadEnterKeyImage();
            ApplyWindowTransparencyForImage();
        }

        private void ApplyWindowTransparencyForImage()
        {
        ...
        /// <summary>默认从嵌入资源加载；exe 同目录的 enter_key_modern.png 可覆盖内置图。</summary>
        private void LoadEnterKeyImage()
        {
            try
            {
                string external = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "enter_key_modern.png");
                if (File.Exists(external))
                {
                    ReplacePictureBoxImage(Image.FromFile(external));
                    return;
                }
                ...
                {
                    if (n.EndsWith("enter_key_modern.png", StringComparison.OrdinalIgnoreCase))
                    {
                        name = n;
                        break;
                    }
                ...
                using (Stream stream = asm.GetManifestResourceStream(name))
                {
                    ...
                    using (var ms = new MemoryStream())
                    {
                        stream.CopyTo(ms);
                        ms.Position = 0;
                        ReplacePictureBoxImage(new Bitmap(ms));
                    }
                }
            }
            catch
        ...
        private void ReplacePictureBoxImage(Image newImage)
        {
